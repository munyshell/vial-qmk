...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
	...
	[_LAY0] = LAYOUT (
      ...
      //|--------+--------+--------+--------+--------+--------|                    |--------+--------+--------+--------+--------+--------|
      //  LCtrl  |    Z   |    X   |    C   |    V   |   B    |                         N   |    M   |  , <   |  . >   |   / ?  |  Rsft  |
      //|--------+--------+--------+--------+--------+--------|					   |--------+--------+--------+--------+--------+--------/
      //  LSft   |ENCODER |   LAlt  |   LAY1 |  SPACE |                               ENTER  |  LAY2  | RAlt	  |        |   Del  |
      //`--------------------------------------------/                              \-------------------------------------------/
	  	//ROW1 
		KC_ESC, KC_W, KC_R, KC_Y, KC_I, KC_P, 									  // ESC | W | R | Y | I | P |
		KC_Q, KC_E, KC_T, KC_U, KC_O, KC_BSPC, 									  //  Q | E | T | U | O | Back |
	    //ROW2 
		KC_TAB, KC_S, KC_F, KC_H, KC_K, KC_SCLN, 							   	  // LAlt | S | F | H | K | ;: |
		KC_A, KC_D, KC_G, KC_J, KC_L, KC_QUOT, 						 			  //   A   | D | G | J | L | '" |
		//ROW3 	
		KC_LCTL, KC_X, KC_V, KC_N, KC_COMM, KC_SLSH, 				 			  // LShift| X | V | N | ,<| /? |
		KC_Z, KC_C, KC_B, KC_M, KC_DOT, KC_RSFT, 				  	 			  //   Z   | C | B | M | .>| RShift
		//ROW4
		KC_LSFT, MO(_LAY1), KC_ENT, KC_RALT, 						 			  // LCtrl|LAY1 | Enter| RAlt
		KC_MPLY, KC_LALT, KC_SPC, MO(_LAY2), KC_DEL),			     			  // Play | Win |  SPC | LAY2 | Del
	...
	[_LAY1] = LAYOUT (
     ...
     //`--------------------------------------------/                              \-------------------------------------------/
		...
		KC_HOME, KC_PGUP, KC_CLCK, KC_DOWN, KC_P1, KC_P3,  			 			  //  HOME | PGUP |CAPSLOCK| DOWN |   1   | 3 |
	    //ROW4	
	...
	[_LAY2] = LAYOUT (
     ...
     //    TAB  |    !      |     @   |   #    |    $   |    %   |                      ^^   |    &   |    *   |   (    |    )   |  Back  |
     //|--------+-----------+---------+--------+--------+--------|                  |--------+--------+--------+--------+--------+--------|
     //   LAlt  |Move2Screen|   XXX   | MC.STOP| MC1.REC|MC1.PLAY|                       -   |    =   |    [   |   ]    |    \   |   ``   |
     //|--------+-----------+---------+--------+--------+--------|                  |--------+--------+--------+--------+--------+--------|
     //   LSft  | TaskMang  |   XXX   | MC.STOP| MC2.REC|MC2.PLAY|                      _    |    +   |    {   |   }    |    |   |   ~~   |
     //|--------+-----------+---------+--------+--------+--------|			        |--------+--------+--------+--------+--------+--------/
     //  LCtrl  | Imp.Pant  |    GUI  |  LAY3  |  SPACE |                              ENTER |        | RAlt   |        |    0   |
     //`------------------------------------------------/                                \-------------------------------------------/
	    //ROW1  
		KC_TAB, KC_AT, KC_DLR, KC_CIRC, KC_ASTR, KC_RPRN, 			 			  //  TAB | @ | $ | ^^ | * |   )  |
		KC_EXLM, KC_HASH, KC_PERC, KC_AMPR, KC_LPRN, KC_BSPC, 					  //    ! | # | % |  & | ( | bACK |
	    //ROW2
		KC_LALT, XXXXXXX, DM_REC1, KC_MINS, KC_LBRC, KC_BSLS, 		 		      //  LAlt |   XXX   | MC1.REC1  | - | [ |  \ |
	    ...
	    //ROW4	
		KC_LCTL, MO(_LAY3), KC_ENT, KC_RALT, 						 		      //   LCtrl  |LAY3 | Enter| RAlt
	...
	[_LAY3] = LAYOUT (
     ...
     //`---------------------------------------------/                             \---------------------------------------------------------------------------------------/
	    //ROW1  
		RESET, XXXXXXX, XXXXXXX, RGB_M_P, RGB_M_R, XXXXXXX, 		 			  //  RESET | XXX | XXX | RGB_M_P | RGB_M_R  | XXX |
...
const rgblight_segment_t PROGMEM _LAY0[]        = RGBLIGHT_LAYER_SEGMENTS(
    {0, 8, 176 ,255, 255}
);
const rgblight_segment_t PROGMEM _LAY1[]     = RGBLIGHT_LAYER_SEGMENTS(
    {0, 8, HSV_BLUE}
);
const rgblight_segment_t PROGMEM _LAY2[]   = RGBLIGHT_LAYER_SEGMENTS(
    {0, 8, 100, 255, 255}
);
const rgblight_segment_t PROGMEM _LAY3[] = RGBLIGHT_LAYER_SEGMENTS(
    {0, 8, HSV_MAGENTA}
);
...
layer_state_t default_layer_state_set_user(layer_state_t state) {
    rgblight_set_layer_state(0, layer_state_cmp(state, _LAY0));
    return state;
};
...
layer_state_t layer_state_set_user(layer_state_t state) {
    ...
    rgblight_set_layer_state(4, layer_state_cmp(state, _LAY3));
    return state;
};
